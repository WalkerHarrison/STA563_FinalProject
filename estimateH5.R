estimateH5 <- function(w, x, y, z, a, pb, q){
  ...
  tmp = sort(a, index.return=TRUE, decreasing = TRUE); sa = tmp$x; ia=tmp$ix 
  ...
  #% Populate histograms
  for (i in 1:leng){
    w = sw[i]; x = sx[i]; y = sy[i]; z = sz[i]; a = sa[i];#print(c(x,y,z))
    his[y,x] = his[y,x]+1; his3[z,y,x] = his3[z, y,x]+1; his4[z,y,x,w] = his4[z,y,x,w]+1; 
    his5[z,y,x,w, a] = his5[z,y,x,w, a]+1
      ...
      for (k in 1:b){
        for (l in 1:b){
          for (i in 1:b){
            for (m in 1:b){
              if (his5[j,k,l,i,m] > 0){
                an = -his5[j,k,l,i, m] * log2(his5[j,k,l,i, m])
                H5 = H5 + an
                }
            }
          }
